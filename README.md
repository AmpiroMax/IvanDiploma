...
## Скрипты чтения и записи

### Входные файлы

- **`iternet/io/ie2d.py`** — парсинг `.dat` (результаты ЭРТ):
  - `parse_ie2d_res(path)` → `IE2DResData` (измерения A, M, N, B, значения)
  - Поддержка 3- и 4-электродной схемы

- **`iternet/io/ie2.py`** — парсинг `.ie2` (геологические модели):
  - `parse_ie2_model(path)` → `IE2Model` (тела, точки, Rho)
